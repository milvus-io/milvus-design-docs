...
- JSON subfield: `{"field": "metadata[\"price\"]", "order": "asc"}`
- Nested JSON: `{"field": "metadata[\"user\"][\"score\"]", "order": "desc"}`
- Dynamic field: `{"field": "age", "order": "desc"}` (automatically resolved)
...
| JSON | Yes | Sort by raw JSON bytes (lexicographic) |
| JSON subfield | Yes | Extract value at JSON path for comparison |
| Dynamic field | Yes | Automatically resolved if not in schema |
...
# Works the same as regular fields
order_by_fields=[
    {"field": "rating", "order": "desc"},
    {"field": "timestamp", "order": "asc"}  # mixing regular and dynamic fields
]
```
...
Nested JSON values can be accessed using bracket notation:

```python
# Single-level JSON path
order_by_fields=[
    {"field": "metadata[\"price\"]", "order": "asc"}
]

# Nested JSON path
order_by_fields=[
    {"field": "metadata[\"user\"][\"score\"]", "order": "desc"}
]
...
- Bool: false < true
- JSON (whole field): Raw byte comparison
...
### JSON Pointer to gjson Path Conversion

JSON Pointer (RFC 6901) uses `/` as separator:
- `/user/name` → `user.name`
- `~0` → `~` (escaped tilde)
- `~1` → `/` (escaped slash)

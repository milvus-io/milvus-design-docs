...
The design uses `ChunkedColumnInterface::Scan` as the expression-facing
abstraction and uses a sparse local file as the Vortex reader's file view.
...
`Scan` is the expression-facing access point for local format data. It returns a
cursor of `ScanBatch` values.
...
`VortexColumn` is a field-level proxy over a shared `VortexColumnGroup`.
...
- Implement `Scan`.
- Implement `Bulk...At` and take helpers for retrieve output.
...
Bitmap or selection pushdown into `Scan` is left as future work. The current
strategy avoids many small reads while keeping semantics simple.
...
The scan API uses `ValidityView` to present nullability uniformly.
...
- `ChunkedColumnInterface::Scan` fallback behavior.
...
- Push offset bitmaps or row selections into `Scan`.

...
**`WithForceDescending` option** — when set, MergeOp forces the output sort direction to descending regardless of input metric types. This is used by decay reranking, where the decay function multiplies scores by factors bounded to [0, 1], ensuring the merged-then-decayed scores are always in a descending-is-better orientation.

#### HybridAssembleOp

Combines results from multiple sub-searches in a hybrid search scenario into per-query DataFrames. This operator handles the assembly of shard-level results before they enter the rerank chain, acting as a bridge between the hybrid search reduce phase and the chain pipeline.
...
Groups rows within each chunk by a field value, keeps the top `groupSize` rows per group, scores each group, then returns the top groups after applying offset/limit.

The intra-group sort direction is configurable via a `sortDescending` field (set through builder-style `WithSortDescending(bool)` setter). This is necessary because not all reranking strategies produce descending scores — for example, weighted reranking with L2 metrics and `normalize=false` produces ascending scores. The rerank builder propagates the correct sort direction from MergeOp's output direction to GroupByOp.
...
**Decay** (`decay`):
```
MergeOp(max, WithForceDescending) -> MapOp(DecayExpr, [field, $score] -> [$score]) -> SortOp($score DESC) -> LimitOp(limit, offset) -> SelectOp(...)
```

Note: `WithForceDescending` is applied to MergeOp because decay multiplies scores by factors in [0, 1], so the output is always descending regardless of the original metric direction.
...
**Import** (`FromSearchResultData`): Converts `schemapb.SearchResultData` into a DataFrame. Each query's results become one chunk. Handles Int64 and String ID types, scores, field data with null bitmaps, and all scalar Milvus types. Nil `FieldsData` entries now return explicit errors rather than being silently skipped, preventing row/column misalignment in the resulting DataFrame.
...
- Timestamptz fields are not supported as decay function inputs. Although Timestamptz is a valid Milvus field type, it is explicitly excluded from decay input validation. Support is deferred to a follow-up change.
- **Breaking change (single-shard):** In single-shard scenarios, score negation now inverts the sort order compared to prior versions. This affects distance metrics (e.g., L2) when combined with grouping. This behavioral difference should be noted in release documentation.
